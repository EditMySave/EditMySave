/**
 * Windblown Save File Codec
 *
 * Format: MTWB header + GZIP(JSON metadata) + gap + GZIP(binary game state)
 *
 * Key discoveries:
 * - Currency values encoded as value << 12 (multiply by 4096) in consecutive uint32 LE fields
 * - 5 currencies stored contiguously: Cogs, Memonite Dust, Memonite Shard, Memonite Fragment, Obsidian Memonite
 * - Located by anchor pattern [00 00 00 68] preceding the currency block
 * - Data offsets shift between saves — must use pattern search, not fixed offsets
 * - Meta flags (unlocks/upgrades) stored as BitSet128 (16 bytes), preceded by 8 zero bytes
 * - Enum value N → bit position N+1 in the bitset
 */
...
const BITSET_ZERO_ANCHOR = new Uint8Array(8); // 8 zero bytes precede the bitset
...
export interface DecodedSave {
  ...
  metaFlags: MetaFlags;
  /** Full original file bytes */
  raw: Uint8Array;
  /** Decompressed binary game state (block 2) */
  block2Decompressed: Uint8Array;
  /** Offset of GZIP block 1 in raw file */
  block1GzipOffset: number;
  /** Offset of GZIP block 2 in raw file */
  block2GzipOffset: number;
  /** Static portion of the gap between block 1 GZIP end and block 2 GZIP start */
  gapStaticBytes: Uint8Array;
  /** Offset of first currency value within decompressed block 2 */
  currencyOffset: number;
  /** Offset of the BitSet128 within decompressed block 2 */
  metaFlagsOffset: number;
}

// ── Compression helpers (node:zlib handles trailing data gracefully) ────
...
/** Decompress a GZIP block. Handles trailing non-GZIP data after the stream. */
function gzipDecompressWithTrailing(data: Uint8Array): Uint8Array {
  return new Uint8Array(inflateRawSync(Buffer.from(data.slice(GZIP_HEADER_SIZE))));
}

/** Decompress a clean GZIP block (no trailing data). */
...
function findCurrencyOffset(block2: Uint8Array): number {
  ...
  while (pos < block2.length - (4 + CURRENCY_COUNT * 4 + 4)) {
    ...
    for (let i = 0; i < CURRENCY_COUNT; i++) {
      ...
      // Lower 12 bits should be zero for clean currency values
      if ((raw & 0xFFF) !== 0) {
        valid = false;
        break;
      }
    ...
    // Validate: slot 5 (index 5, after the 5 currencies) should be zero
...
function findMetaFlagsOffset(block2: Uint8Array, searchFrom: number): number {
  // The bitset is preceded by 8 zero bytes, located in the tail section
  for (let i = searchFrom; i < block2.length - (8 + BITSET_SIZE); i++) {
    let allZero = true;
    for (let j = 0; j < 8; j++) {
      if (block2[i + j] !== 0) { allZero = false; break; }
    }
    if (!allZero) continue;

    // Check the 16 bytes after the zeros have a reasonable number of set bits
...
function bitPositionForEnum(enumVal: number): number {
  return enumVal + FLAG_BIT_OFFSET;
}
...
function readMetaFlags(block2: Uint8Array, offset: number): MetaFlags {
  ...
  for (let i = 0; i < META_FLAG_NAMES.length; i++) {
    ...
    const bitPos = bitPositionForEnum(enumVal);
...
function writeMetaFlags(block2: Uint8Array, offset: number, flags: MetaFlags): void {
  // Clear the bitset first (preserve bits outside our mapped range)
  // Actually, only modify bits we know about — leave unmapped bits untouched
  for (let i = 0; i < META_FLAG_NAMES.length; i++) {
...
export async function decodeSave(filePath: string): Promise<DecodedSave> {
  ...
  // Block 1 has trailing gap bytes after its GZIP stream, so use raw deflate.
  ...
  // GZIP trailer: CRC32 (4B) + ISIZE (4B). ISIZE = decompressedSize mod 2^32.
  ...
  for (let i = block2GzipOffset - 4; i >= block1GzipOffset; i--) {
    if (raw[i] === isizeBytes[0] && raw[i + 1] === isizeBytes[1] &&
        raw[i + 2] === isizeBytes[2] && raw[i + 3] === isizeBytes[3]) {
      block1GzipEnd = i + 4;
      break;
    }
  }
...
export async function encodeSave(
  ...
  const SHA1_SIZE = 20;
  const HEADER_HASH_OFFSET = 0x09;

  // 1. Modify block 2 in-place with updated currencies and flags
  const modifiedBlock2 = new Uint8Array(decoded.block2Decompressed);
  writeCurrencies(modifiedBlock2, decoded.currencyOffset, decoded.currencies);
  writeMetaFlags(modifiedBlock2, decoded.metaFlagsOffset, decoded.metaFlags);

  // 2. Recompress block 2
  const compressedBlock2 = gzipCompress(modifiedBlock2);

  // 3. Preserve the original block 1 compressed GZIP data (unchanged)
  //    Find block 1's actual GZIP end using ISIZE search
  const block1Decompressed = gzipDecompressWithTrailing(
    decoded.raw.slice(decoded.block1GzipOffset, decoded.block2GzipOffset),
  );
  const isizeBytes = new Uint8Array(4);
  new DataView(isizeBytes.buffer).setUint32(0, block1Decompressed.length & 0xFFFFFFFF, true);
  let block1GzipEnd = -1;
  for (let i = decoded.block2GzipOffset - 4; i >= decoded.block1GzipOffset; i--) {
    if (decoded.raw[i] === isizeBytes[0] && decoded.raw[i + 1] === isizeBytes[1] &&
        decoded.raw[i + 2] === isizeBytes[2] && decoded.raw[i + 3] === isizeBytes[3]) {
      block1GzipEnd = i + 4;
      break;
    }
  }
  ...
  const block1Compressed = decoded.raw.slice(decoded.block1GzipOffset, block1GzipEnd);

  // 4. Compute SHA-1 hashes
  //    Header hash (0x09-0x1C): SHA-1(block1 compressed GZIP data)
  const headerHash = sha1(block1Compressed);

  //    Gap hash: SHA-1(gapStaticBytes + block2 compressed GZIP data)
  const gapHashInput = new Uint8Array(decoded.gapStaticBytes.length + compressedBlock2.length);
  gapHashInput.set(decoded.gapStaticBytes, 0);
  gapHashInput.set(compressedBlock2, decoded.gapStaticBytes.length);
  const gapHash = sha1(gapHashInput);

  // 5. Build the file:
  //    [magic 4B] [version 5B] [headerHash 20B] [block1 gzip] [gapHash 20B] [gapStatic] [block2 gzip]
  const magic = decoded.raw.slice(0, 4);
  const version = decoded.raw.slice(4, 9);

  const totalSize = 4 + 5 + SHA1_SIZE + block1Compressed.length +
    SHA1_SIZE + decoded.gapStaticBytes.length + compressedBlock2.length;
  const output = new Uint8Array(totalSize);
  let offset = 0;

  output.set(magic, offset); offset += 4;
  output.set(version, offset); offset += 5;
  output.set(headerHash, offset); offset += SHA1_SIZE;
  output.set(block1Compressed, offset); offset += block1Compressed.length;
  output.set(gapHash, offset); offset += SHA1_SIZE;
  output.set(decoded.gapStaticBytes, offset); offset += decoded.gapStaticBytes.length;
  output.set(compressedBlock2, offset);

  await Deno.writeFile(outputPath, output);
}

// ── JSON conversion for CLI ─────────────────────────────────────────────
...
export function toEditableJSON(decoded: DecodedSave): EditableJSON {
  return {
    playerName: decoded.metadata.PlayerName,
    currencies: { ...decoded.currencies },
    metaFlags: { ...decoded.metaFlags },
  };
}

export function applyEdits(decoded: DecodedSave, edits: EditableJSON): void {
  decoded.currencies = { ...edits.currencies };
  decoded.metaFlags = { ...edits.metaFlags };
}
