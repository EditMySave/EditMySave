// Far Far West save file encoder/decoder
//
// Format (verified against experiments/far-far-west/far-far-west-editor):
...
/** Extract the SteamID (leading digits) from a save filename. */
export function steamIdFromPath(path: string): string {
  const stem = path.split(/[\\/]/).pop()!.replace(/\.save$/i, "");
  const m = stem.match(/^(\d+)/);
  if (!m) {
    throw new Error(`save filename must start with a SteamID: ${stem}`);
  }
  return m[1];
}

/** Derive the AES-256 key from a seed string (SteamID + PARTY_SUFFIX). */
...
/** Decrypt a save file's bytes. Returns the raw GVAS plaintext. */
export function decryptSave(data: Uint8Array, key: Uint8Array): Uint8Array {
  ...
  if (pt[0] !== 0x47 || pt[1] !== 0x56 || pt[2] !== 0x41 || pt[3] !== 0x53) {
    throw new Error(
      `decrypt produced non-GVAS magic: ${Buffer.from(pt.subarray(0, 8)).toString("hex")}`,
    );
  }
  return new Uint8Array(pt);
}

/** Encrypt GVAS plaintext. Pads with 0x00 to a 16-byte multiple. */
export function encryptSave(
  ...
  const useIv = iv ?? new Uint8Array(randomBytes(16));
  if (useIv.length !== 16) throw new Error("iv must be 16 bytes");
  const cipher = createCipheriv("aes-256-cbc", key, useIv);
...
function bytesToBase64(b: Uint8Array): string {
  // chunk to avoid blowing the call stack on String.fromCharCode(...)
  let s = "";
  const CHUNK = 0x8000;
...
class Reader {
  ...
  /** Heuristic: would offset `pos` plausibly start a valid fstring? */
...
class Writer {
  ...
  fstring(s: string) {
    ...
    // Try ASCII first (matches the Python writer's behavior).
    let isAscii = true;
...
function writeStructValue(w: Writer, structType: string, value: Any): void {
  switch (structType) {
    case "Vector":
    case "Vector3f": {
      // Reader picks f32 vs f64 by size; Python's writer always uses f32.
      // We keep the f32 default; oversized values would have been parsed as
      // f64 only when size==24, and we'd round-trip via _value_trailing.
...
export function parseGvas(data: Uint8Array): GvasFile {
  const r = new Reader(data);
  const header = readHeader(r);

  // The save has a single zero byte between the class name and the first
  // property tag. Preserve it verbatim.
...
export function serializeGvas(g: GvasFile): Uint8Array {
  const w = new Writer();
...
interface GvasJson {
  header: GvasHeader;
  pre_properties: string; // hex
  properties: Any[];
  trailing: string;       // hex
}

function gvasToJson(g: GvasFile): string {
  const obj: GvasJson = {
    header: g.header,
    pre_properties: bytesToHex(g.pre_properties),
    properties: g.properties,
    trailing: bytesToHex(g.trailing),
  };
  return JSON.stringify(obj, null, 2);
}

function jsonToGvas(text: string): GvasFile {
  const obj = JSON.parse(text) as GvasJson;
  return {
    header: obj.header,
    pre_properties: hexToBytes(obj.pre_properties ?? ""),
    properties: obj.properties,
    trailing: hexToBytes(obj.trailing ?? ""),
  };
}

/**
 * Decode encrypted save -> full scaffolded GVAS JSON.
 *
 * Includes every internal field (type tags, _meta_raw, FName suffixes, etc.)
 * so a parse -> serialize round trip is byte-identical. Use this for
 * structural edits (adding inventory entries, new map keys). For everyday
 * value tweaks, prefer the friendly form (`decodeToFriendlyJSON`).
 */
export async function decodeToRawJSON(savePath: string, jsonPath: string): Promise<void> {
  const seed = steamIdFromPath(savePath) + PARTY_SUFFIX;
  const key = await deriveKey(seed);
  const data = await Deno.readFile(savePath);
  const plaintext = decryptSave(data, key);
  const gvas = parseGvas(plaintext);
  await Deno.writeTextFile(jsonPath, gvasToJson(gvas));
  console.log(`✓ Decoded (raw) ${gvas.properties.length} top-level properties to ${jsonPath}`);
}

/**
 * Encode a raw scaffolded GVAS JSON -> encrypted save. Round-trips byte-identical
 * when the JSON is untouched. SteamID/IV taken from sourceSave (default) or supplied directly.
 */
export async function encodeFromRawJSON(
  jsonPath: string,
  outputPath: string,
  sourceSave?: string,
  iv?: Uint8Array,
  ...
  const gvas = jsonToGvas(text);
  const steamId = steamIdFromPath(sourceSave ?? outputPath);
  const key = await deriveKey(steamId + PARTY_SUFFIX);
  const plaintext = serializeGvas(gvas);
  const out = encryptSave(plaintext, key, iv);
  await Deno.writeFile(outputPath, out);
  console.log(`✓ Encoded (raw) ${gvas.properties.length} properties -> ${outputPath} (${out.length} bytes)`);
}

// ---------------------------------------------------------------------------
// Friendly JSON (v2)
//
// Strips GVAS scaffolding for everyday editing. The original save is the
// source of truth for structure on encode: we re-parse it, then graft the
// user's value edits onto the scaffold. This means friendly mode does NOT
// support adding/removing inventory entries, map keys, or array items —
// use the raw helpers above for that.
// ---------------------------------------------------------------------------

const FNAME_SUFFIX_RE = /_\d+_[0-9A-Fa-f]{32}$/;

/** Strip the UE FName `_<index>_<32-hex>` suffix from a property name. */
export function stripFNameSuffix(name: string): string {
  return name.replace(FNAME_SUFFIX_RE, "");
}

/** Looks like a property list (rather than a fixed-layout struct). */
...
/** Inventory-style array entry: exactly `name` (NameProperty) + `amount` (IntProperty). */
...
/** Friendly view of a parsed GVAS file. Drops header/pre_properties/trailing. */
...
function friendlyStruct(structType: string, value: Any, path: string, unparsed: Record<string, string>): Any {
  if (isPropertyList(value)) return friendlyFromProperties(value, path, unparsed);
  // Fixed-layout structs (Vector, Guid, ...) — already a plain shape; pass through.
  return value;
}

function friendlyArray(value: Any[], meta: TypeMeta, path: string, unparsed: Record<string, string>): Any {
  // Inventory-style flattening: ArrayProperty<StructProperty> where every entry
  // is a {name, amount} property pair. Render as { nameValue: amountValue }.
  if (
  ...
  // Plain inline array: render each element.
...
function friendlyMap(value: Any, meta: TypeMeta, path: string, unparsed: Record<string, string>): Any {
  ...
  // Only flatten if every key is a scalar (string/number/bool).
...
/** Decode encrypted save -> friendly editable JSON (default decode path). */
export async function decodeToFriendlyJSON(savePath: string, jsonPath: string): Promise<void> {
  const seed = steamIdFromPath(savePath) + PARTY_SUFFIX;
  const key = await deriveKey(seed);
  const data = await Deno.readFile(savePath);
  const plaintext = decryptSave(data, key);
  const gvas = parseGvas(plaintext);
  const { tree, unparsed } = toFriendly(gvas);
  const out: Record<string, Any> = { ...tree };
  if (Object.keys(unparsed).length) out._unparsed = unparsed;
  await Deno.writeTextFile(jsonPath, JSON.stringify(out, null, 2));
  console.log(`✓ Decoded ${gvas.properties.length} top-level properties to ${jsonPath}`);
}

// ---------------------------------------------------------------------------
// Graft on encode: re-parse the original save, patch values in place from
// the friendly JSON, serialize, encrypt.
// ---------------------------------------------------------------------------
...
function graftProperties(props: Any[], friendly: Any, path: string): void {
  ...
  for (const [name, v] of Object.entries(friendly)) {
    ...
    if (p._parse_error) continue; // leave the raw bytes alone
...
function graftValue(p: Any, friendly: Any, path: string): void {
  ...
  switch (type_) {
    case "StructProperty": {
      if (isPropertyList(p.value)) {
        graftProperties(p.value, friendly, path);
      } else {
        p.value = friendly;
      }
      return;
    ...
    case "SetProperty": {
      if (friendly && Array.isArray(friendly.items)) {
        ...
        if (friendly.items.length !== existing.length) {
          throw new Error(`set length mismatch at ${path}: friendly mode cannot add/remove items (have ${existing.length}, want ${friendly.items.length})`);
        }
...
function graftArray(p: Any, meta: TypeMeta, friendly: Any, path: string): void {
  ...
  // Inventory shape: friendly is a flat object {name: amount, ...}; locate each
  // scaffold entry by its `name` property and overwrite its `amount`.
  if (
    meta.inner_type === "StructProperty" &&
    value.length > 0 &&
    value.every(isInventoryEntry) &&
    ...
    for (const key of Object.keys(friendly)) {
      if (!seen.has(key)) {
        console.warn(`graft: inventory key "${key}" at ${path} not present in original save — skipping (friendly mode can't add entries)`);
      }
    }
    return;
  }
  // Plain array: length must match.
  ...
  if (friendly.length !== value.length) {
    throw new Error(`array length mismatch at ${path}: friendly mode cannot add/remove items (have ${value.length}, want ${friendly.length})`);
  }
...
function graftMap(p: Any, meta: TypeMeta, friendly: Any, path: string): void {
  ...
  // Scalar-keyed flattened friendly object: { key: value, ... }
  ...
  // Unflattened form: { items: [{key, value}], num_keys_to_remove? }
  if (friendly && Array.isArray(friendly.items)) {
    if (friendly.items.length !== items.length) {
      throw new Error(`map length mismatch at ${path}: friendly mode cannot add/remove keys`);
    }
...
function graftInline(type_: string, meta: TypeMeta, original: Any, friendly: Any, path: string): Any {
  if (type_ === "StructProperty") {
...
/** Encode a friendly-format JSON -> encrypted save by grafting onto the original save's scaffold. */
export async function encodeFromFriendlyJSON(
  jsonPath: string,
  outputPath: string,
  ...
  const text = await Deno.readTextFile(jsonPath);
  const friendly = JSON.parse(text) as Any;
  // strip the user-visible sidekey before grafting
  const root = { ...friendly };
  delete root._unparsed;

  const original = await Deno.readFile(originalSavePath);
  const steamId = steamIdFromPath(originalSavePath);
  const key = await deriveKey(steamId + PARTY_SUFFIX);
  const plaintextOrig = decryptSave(original, key);
  const gvas = parseGvas(plaintextOrig);

  graftProperties(gvas.properties, root, "");

  const plaintext = serializeGvas(gvas);
  const out = encryptSave(plaintext, key, iv);
  await Deno.writeFile(outputPath, out);
  console.log(`✓ Encoded ${gvas.properties.length} properties -> ${outputPath} (${out.length} bytes)`);
}

/** Round-trip integrity check: decrypt+parse+reserialize must be byte-identical to the decrypted plaintext. */
export async function roundtripCheck(savePath: string): Promise<{ ok: boolean; firstDiff?: number; size?: { orig: number; rebuilt: number } }> {
  const seed = steamIdFromPath(savePath) + PARTY_SUFFIX;
  const key = await deriveKey(seed);
  const data = await Deno.readFile(savePath);
  const pt = decryptSave(data, key);
  const rebuilt = serializeGvas(parseGvas(pt));
  if (rebuilt.length !== pt.length) {
    return { ok: false, size: { orig: pt.length, rebuilt: rebuilt.length } };
  }
  for (let i = 0; i < pt.length; i++) {
    if (pt[i] !== rebuilt[i]) return { ok: false, firstDiff: i };
  }
  return { ok: true };
}
