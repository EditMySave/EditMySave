...
export interface ProfileFileData {
  ...
  joker_usage: Partial<Record<string, JokerStats>>
  MEMORY: {
    deck: string
    stake: number
  }
  deck_stakes: Record<string, unknown>
  voucher_usage: Partial<Record<string, Usage>>
  stake: number
  challenge_progress: {
    unlocked: Record<string, unknown>
    completed: Record<string, unknown>
  }
  consumeable_usage: Partial<Record<string, Usage>>
  hand_usage: Partial<Record<string, Usage>>
  deck_usage: Partial<Record<string, number>>
  progress: {
